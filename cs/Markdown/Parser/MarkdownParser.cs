...
public class MarkdownParser: IParser
{
    ...
    private static void ProcessUnpairedTag(string text, TagType currentTag, Stack<OpenToken> tokensWithOpenTag, int position,
        List<Token> result)
    {
        if (tags[currentTag].IsPairedTag)
        {
    ...
    private static void ProcessTagEndOfLine(string text, Stack<OpenToken> tokensWithOpenTag, int position, List<Token> result)
    {
    ...
    private static void ProcessPairedTag(string text, TagType currentTag, Stack<OpenToken> tokensWithOpenTag, int position, List<Token> result)
    {
        if (!tags[currentTag].IsPairedTag)
        {
    ...
    /// </returns>
    private static bool CheckOpenTokenTagBoldOrItalicLocatedInsideWords(string text, int endPosition, OpenToken openToken)
    {
        if (openToken.OpenTagType != TagType.Bold && openToken.OpenTagType != TagType.Italic)
        {
            return false;
        }
        ...
        var isStartTagInMiddleWord = symbolBeforeStartTag >= 0 && !char.IsWhiteSpace(text[openToken.TextStartPosition]) && !char.IsWhiteSpace(text[symbolBeforeStartTag]);
        var isEndTagInMiddleWord =  endPosition + tagLength < text.Length && !char.IsWhiteSpace(text[endPosition + tagLength]) && !char.IsWhiteSpace(text[endPosition - 1]);
        ...
        if ((isStartTagInMiddleWord || isEndTagInMiddleWord) && textContainsSeveralWords)
        {
            return true;
        }
        return false;
    ...
    private static List<Token> ProcessBoldTagInsideItalicTag(List<Token> tokens)
    {
        ...
        {
            result[i] = SearchForTokensTagItalicAndConvertTokensTagBold(result[i]);
        }
        return result;
    }
    ...
    private static Token SearchForTokensTagItalicAndConvertTokensTagBold(Token token)
    {
        if (token is { TagType: TagType.Italic, Children: not null })
        {
            ...
            {
                result.Children!.Add(ConvertTokensTagBoldToTokensTagNone(child));
            }
            return result;
        }
    ...
    private static List<Token> AddUnfinishedTags(string text, OpenToken? openTokenWithEmptyTag, Stack<OpenToken> tokensWithOpenTag, List<Token> listTokens)
    {
    ...
    private static TagType GetTagType(string text, int position, Stack<OpenToken> tokensWithOpenTag)
    {
        ...
        foreach (var keyValuePair in tags)
        {
            ...
            if (tagLength + position > text.Length)
            {
                continue;
            }
        ...
        return possibleTags.Count == 0 ? TagType.None : possibleTags.OrderByDescending(x => x.Value.Content.Length).First().Key;
    }
        ...
        KeyValuePair<TagType, Tag> keyValuePair, Dictionary<TagType, Tag> possibleTags)
    {
        switch (keyValuePair.Key)
        {
            case TagType.Header:
            {
                if (IsHeader(text, position, tokensWithOpenTag))
                {
                    possibleTags.Add(keyValuePair.Key, keyValuePair.Value);
                }
    ...
    private static bool IsHeader(string text, int position, Stack<OpenToken> tokensWithOpenTag)
    {
        ...
        var isNewParagraph = position == 0 || (position >= 2 && text.AsSpan(position - 2, 2).Equals("\n\n", StringComparison.Ordinal));
        var isStackContainsCurrentTag = tokensWithOpenTag.Select(a => a.OpenTagType).Contains(TagType.Header);
        
        return text.AsSpan(position, tagLength).Equals(tagContent, StringComparison.Ordinal) && isNewParagraph && !isStackContainsCurrentTag;
    }
